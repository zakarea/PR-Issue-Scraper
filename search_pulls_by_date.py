...
def make_query_count_issues(date):
    return """
{
  search(query: "Android created:<DATE is:merged is:pr linked:issue", type: ISSUE, first: 1) {
    issueCount
  }
}
    ...
    return """
{
  ...
  search(query: "Android created:DATE language:kotlin is:merged is:pr sort:reactions", type: ISSUE, first: 30, after: AFTER) {
...
def remove_unlinked_pulls(data):
    ...
    if "data" in data and "nodes" in data["data"]["search"]:
        for i in range(len(data["data"]["search"]["nodes"])):
            body_text = data["data"]["search"]["nodes"][i]["bodyText"]
            # print(comment_text)
            search = re.search(pattern, body_text)

            if search is not None:
                print(search)

            if "comments" in data["data"]["search"]["nodes"][i] and "nodes" in data["data"]["search"]["nodes"][i]["comments"]:
                for j in range(len(data["data"]["search"]["nodes"][i]["comments"]["nodes"])):
                    comment_text = data["data"]["search"]["nodes"][i]["comments"]["nodes"][j]["bodyText"]
                    # print(comment_text)
                    search = re.search(pattern, comment_text)
                    if search is not None:
                        print(search)
    ...
    #             return remove_unlinked_pulls(data)
...
if __name__ == "__main__":
    ...
    fileName = "android_pull_request_" + str_start_date + "_" + str_end_date + ".md"
